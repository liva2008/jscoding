...
BlocklyStorage.backupProgram = function(workspace, program) {

  if ('localStorage' in window) {
    var xml = Blockly.Xml.workspaceToDom(workspace);
    var programs = JSON.parse(window.localStorage.getItem("BlocklyCode")) || {};
	/*
	if(programs.hasOwnProperty(program)){
		if(confirm("该文件已存在，需要覆盖吗？")){
			programs[program] = Blockly.Xml.domToText(xml);
			window.localStorage.setItem("BlocklyCode", JSON.stringify(programs));
		}
	}
	else{
		programs[program] = Blockly.Xml.domToText(xml);
...
BlocklyStorage.backupProgromOnUnload = function(opt_workspace, program) {
  var workspace = opt_workspace || Blockly.getMainWorkspace();
  window.addEventListener('unload',
      function() {BlocklyStorage.backupProgram(workspace, program);}, false);
};
...
BlocklyStorage.restoreProgram = function(opt_workspace, program) {
  if ('localStorage' in window && window.localStorage["BlocklyCode"]) {
    var workspace = opt_workspace || Blockly.getMainWorkspace();
	var pro = JSON.parse(window.localStorage.getItem("BlocklyCode"));
	if(pro[program]){
		var xml = Blockly.Xml.textToDom(pro[program]);
		Blockly.Xml.domToWorkspace(xml, workspace);
	}
  }
};
